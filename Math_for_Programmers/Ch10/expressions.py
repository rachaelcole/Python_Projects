...
def package(maybe_expression):
    if isinstance(maybe_expression, Expression):
        ...
        raise ValueError(f"Can't convert {maybe_expression} to an expression.")
...
# Abstract Base Class: Expression(ABC)
class Expression(ABC):
    ...
    def python_function(self,**bindings):
        # code = "lambda {}:{}".format(', '.join(sorted(distinct_variables(self))), self._python_expr())
        global_vars = {"math":math}
        return eval(self._python_expr(), global_vars, bindings)

# Power class: call Power(Variable('x'),Number(2)) for x**2, etc.
class Power(Expression):
    def __init__(self,base,exponent):
        self.base = base
        self.exponent = exponent
    def latex(self):
        return "{} ^ {{ {} }}".format(paren_if_instance(self.base, Sum, Negative, Difference, Quotient, Product), self.exponent.latex())
    def evaluate(self, **bindings):
...
# Product combinator class: e.g. for 3x**2 call Product(Number(3), Power(variable('x'), Number(2)))
class Product(Expression):
    def __init__(self, exp1, exp2):
        self.exp1 = exp1
        self.exp2 = exp2
    def latex(self):
        return "{}{}".format(paren_if_instance(self.exp1, Sum, Negative, Difference),
...
# Sum combinator class:
class Sum(Expression):
    def __init__(self, *exps):  # Allows any number of terms to be summed
        self.exps = exps
    def latex(self):
...
class Quotient(Expression):
    ...
    def derivative(self, var):
        return Quotient(Difference(
            ...
            Product(self.numerator, self.denominator.derivative(var))), Power(self.denominator, Number(2)))
    def _python_expr(self):
        return "({}) / ({})".format(self.exp1._python_expr(), self.exp2._python_expr())

# Difference combinator class:
...
# Negative combinator class:
class Negative(Expression):
    def __init__(self, exp):
        self.exp = exp
    def latex(self):
        return "- {}".format(paren_if_instance(self.exp, Sum, Difference, Negative))
    def evaluate(self, **bindings):
...
class Function(Expression):
    def __init__(self, name, make_latex=None):  # Stores a string that is the function's name (like 'sin')
        self.name = name
...
# Stores a function and the argument it is applied to in an Apply class:
class Apply(Expression):
    ...
    def derivative(self,var):
        return Product(self.argument.derivative(var), _derivatives[self.function.name].substitute(_var, self.argument))
    def substitute(self, var, exp):
    ...
    def display(self):
        return self.function.display()

# Set constants, more helper functions:
_function_bindings = {
    "sin": math.sin,
    "cos": math.cos,
    "ln": math.log,
