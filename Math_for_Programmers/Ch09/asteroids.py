...
class PolygonModel():
    ...
    def move(self, milliseconds):
        # The change in position x is dx
        dx, dy = self.vx * milliseconds / 1000.0, self.vy * milliseconds / 1000.0
        # Complete the movement for the frame
...
acceleration = 3  # set acceleration magnitude constant
def main():
    pygame.init()
    ...
    while not done:
        ...
        if keys[pygame.K_UP]:
            # Calculate ax,ay based on the fixed magnitude of acceleration and the angle of the ship
            ax = acceleration * cos(ship.rotation_angle)
            ay = acceleration * sin(ship.rotation_angle)
            # Update x and y velocities by ax*t, ay*t
