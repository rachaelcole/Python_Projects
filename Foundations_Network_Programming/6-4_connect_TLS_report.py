...
# A script that makes an encrypted connection using TLS and then reports on its features using:
    # getpeercert() - returns a dictionary of fields picked out of the peer's certificate
    # cipher() returns the name of the cipher currently in use over the connection
    # compression() returns the name of the compression algorithm in use
    # Uses ctypes to try to learn the TLS protocol in use
...
def open_tls(context, address, server=False):
    ...
    if server:
        ...
        raw_sock.listen(1)
        print('Interface where we are listening', address)
        raw_client_sock, address = raw_sock.accept()
        print('Client has connected from address', address)
        return context.wrap_socket(raw_client_sock, server_side=True)
    else:
        print('Address we want to talk to: ', address)
...
def describe(ssl_sock, hostname, server=False, debug=False):
    ...
    if cert is None:
        print('Peer certificate: none')
    else:
        print('Peer certificate: provided')
        ...
        print('Name(s) on peer certificate', *names or ['none'])
        if (not server) and names:
            ...
            message = 'Yes'
        print('Whether name(s) match the hostname: ', message)
        for category, count in sorted(context.cert_store_stats().items()):
            print('Certificates loaded of type: {}'.format(category), count)
        try:
            ...
            print('Protocol version negotiated:', protocol_version)
        cipher, version, bits = ssl_sock.cipher()
        compression = ssl_sock.compression()
        print('Cipher chosen for this connection:', cipher)
        print('Cipher defined in TLS version:', version)
        print('Cipher key has this many bits: ', bits)
        print('Compression algorithm in use:', compression or 'none')
...
class PySSLSocket(ctypes.Structure):
    ...
    _fields_ = [('ob_refcnt', ctypes.c_ulong), ('ob_type', ctypes.c_void_p), ('Socket', ctypes.c_void_p), ('ssl', ctypes.c_void_p)]
...
def lookup(prefix, name):
    ...
    try:
        ...
        message = f'Error! {name} is not one of the available names:\n', ' '.join(sorted(matching_names))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', metavar='certfile', default=None, help='path to PEM file with client certificate')
    parser.add_argument('-C', metavar='ciphers', default='ALL', help='list of ciphers formatted per OpenSSL')
    parser.add_argument('-p', metavar='PROTOCOL', default='SSLv23', help='protocol version (default SSLv23)')
    parser.add_argument('-s', metavar='certfile', default=None, help='run as server: path to certificate PEM file')
    parser.add_argument('-d', action='store_true', default=False, help='debug mode: do not hide "ctypes" exceptions')
    parser.add_argument('-v', action='store_true', default=False, help='verbose: print out remote certificate')
