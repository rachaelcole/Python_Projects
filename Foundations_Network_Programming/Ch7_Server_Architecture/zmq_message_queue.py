# A small zero message queue messaging fabric linking 5 different workers/message queues
...
def bitsource(zcontext, url):
    # Produces random points in the unit square
    zsock = zcontext.socket(zmq.PUB)
    zsock.bind(url)
    while True:
        zsock.send_string(ones_and_zeroes(B * 2))
        time.sleep(0.01)


def always_yes(zcontext, in_url, out_url):
    # Coordinates in the lower-left (0,0) quadrant are inside the unit circle
