...
# blackjack.py - A card game using ASCII art where the user tries to get as close to 21 points as possible without
# going over 21
...
On your first turn, you can (D)ouble down to increase your bet, but must hit exactly one more time before standing.
...
The dealer stops hitting at 17.""")
    ...
    while True:  # Main game loop
        ...
        if dealer_value > 21:
            ...
            print(f'You lost! Dealer takes ${bet}.')
            money -= bet
        elif player_value > dealer_value:
            print(f'You won ${bet}!')
        elif player_value == dealer_value:
            print(f"It's a tie! ${bet} is returned to you.")
...
def get_hand_value(cards):
    ...
    for card in cards:
        ...
        if rank == 'A':
            num_aces += 1
        elif rank in ('K', 'Q', 'J'):  # Face cards are worth ten points
            value += 10
        else:
            value += int(rank)  # Numbered cards are worth their number

    # Add the value for the aces
    value += num_aces  # Add 1 per ace
...
def display_cards(cards):
    ...
    for i, card in enumerate(cards):
        ...
        if card == BACKSIDE:
            ...
            rank, suit = card  # The card is in a tuple
...
def get_move(player_hand, money):
    ...
    while True:  # Keep looping until the player enters a correct move
        ...
        moves = ['(H)it', '(S)tand']
        # The player can double down on their first move, when they will have exactly two cards:
